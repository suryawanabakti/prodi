...
export default function ProfilLulusanEdit({ profilLulusan }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    judul: profilLulusan.judul,
    deskripsi: profilLulusan.deskripsi,
    gambar: null as File | null,
    urutan: profilLulusan.urutan,
    is_active: profilLulusan.is_active,
    _method: "PUT",
  })

  const [previewImage, setPreviewImage] = useState<string | null>(profilLulusan.gambar_url)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    post(route("admin.profil-lulusan.update", profilLulusan.id))
  }

  return (
    <AppLayout>
      <Head title="Edit Profil Lulusan" />
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Edit Profil Lulusan</h1>
            <p className="text-muted-foreground">Edit profil lulusan program studi</p>
          </div>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>Form Edit Profil Lulusan</CardTitle>
          </CardHeader>
          <CardContent>
